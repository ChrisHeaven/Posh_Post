    ...
    try {
        ...
        if (!con.isClosed()) {
            ...
            if (rs.next() == false) {
                ...
                sql = "select upvote_number from test.weibo where username = '" + username + "'" + "or username in (select friends from test.relations, test.users_state where state = 1 and relations.username = users_state.username) order by post_time desc";
    ...
    try {
        ...
        if (!con.isClosed()) {
            ...
            if (rs.next() == false) {
                ...
                sql = "select upvote_number from test.weibo order by upvote_number desc";
                PreparedStatement ptmt = (PreparedStatement)con.prepareStatement(sql);
                ResultSet rss = ptmt.executeQuery();
                int i = 0, j;
                while (rss.next()) {
