    ...
    try {
        ...
        if (!con.isClosed()) {
            ...
            if (rs.next() == false) {
                ...
                sql = "select sex, introduction from users_information where username in (select username from test.users_state where username not in (select friends from test.relations, test.users_state where state=1 and relations.username=users_state.username) and username not in (select username from test.users_state where state=1))";
