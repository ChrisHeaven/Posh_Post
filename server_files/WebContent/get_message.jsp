    ...
    try {
        ...
        if (!con.isClosed()) {
            ...
            if (rs.next() == false) {
                ...
                sql = "select message_content from message_content where id in (select id from message_content where sender = '" + username + "' and receiver = '" + friend + "') or id in (select id from message_content where receiver = '" + username + "' and sender = '" + friend + "')";

                ResultSet rss = stmt.executeQuery(sql);
                int i = 0, j;
                while (rss.next()) {
                    outcome[i] = rss.getString("message_content").toString();
                    i++;
                }
                for (j = 0; j < i; j++){
