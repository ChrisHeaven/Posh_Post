    ...
    try {
        ...
        if (!con.isClosed()) {
            ...
            if (rs.next() == false) {
                ...
                sql = "select content from test.weibo where username = '" + username + "'" + "or username in (select friends from test.relations, test.users_state where state=1 and relations.username=users_state.username) order by post_time desc";
                PreparedStatement ptmt = (PreparedStatement)con.prepareStatement(sql);
                ResultSet rss = ptmt.executeQuery();
                int i = 0, j;
                while (rss.next()) {
                    outcome[i] = rss.getString("content").toString();
                    i++;
                }
                for (j = 0; j < i; j++){
    ...
    try {
        ...
        if (!con.isClosed()) {
            ...
            if (rs.next() == false) {
                ...
                sql = "select content from test.weibo order by upvote_number desc";
                PreparedStatement ptmt = (PreparedStatement)con.prepareStatement(sql);
                ResultSet rss = ptmt.executeQuery();
                int i = 0, j;
                while (rss.next()) {
